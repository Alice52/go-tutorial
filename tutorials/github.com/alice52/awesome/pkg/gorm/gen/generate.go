...
func main() {
	...
	g := gen.NewGenerator(gen.Config{
		...
		FieldCoverable: false, // generate pointer when field has default value, to fix problem zero value cannot be assign:
	...
	jsonField := gen.FieldJSONTagWithNS(func(columnName string) (tagContent string) {
		toStringField := `balance, ` //     `balance` decimal(11,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '余额',
	...
	User := g.GenerateModel("user",
		append(
			...
			gen.FieldRelate(field.HasMany, "Address", Address,
				&field.RelateConfig{GORMTag: map[string][]string{"foreignKey": {"UID"}}}),
		)...,
	)
	Address = g.GenerateModel("address",
		append(
			fieldOpts,
			gen.FieldRelate(field.BelongsTo, "User", User,
				&field.RelateConfig{GORMTag: map[string][]string{"foreignKey": {"UID"}}}),
		)...,
	)
